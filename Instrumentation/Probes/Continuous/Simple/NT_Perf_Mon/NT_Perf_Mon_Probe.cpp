...
/** @file	NT_Perf_Mon_Probe.cpp
 *  @ingroup	NTPerfMonProbe
 *  @brief Contains implementation of a Windows NT System Monitor Probe.
 *
 * This file provides implementation of a Windows NT System Monitor 
 * Continuous Probe that 
...
/// RCS version
static const char* version = "$Id: NT_Perf_Mon_Probe.cpp,v 1.13 2001/12/27 20:35:18 spatil Exp $";
...
worker(void *args)
{
	// The counter name should be passed as 
	//  "\System\% Total Processor Time" and not as 
	// "\\System\\% Total Processor Time" on the command line.
  
  int count,i;
  char duration[1024], *data = new char[BUFSIZ+8192], 
	                   *counter_dup = new char[1024];
  char *tmp = data;
  ...
  for ( i = 0; args_pa[i] != 0 ; i++)
   {
     if (ACE_OS::strcmp(args_pa[i],"-r") == 0)
      ...
      else if (ACE_OS::strcmp(args_pa[i],"-n") == 0)
      {
        strcpy(counter_dup, args_pa[++i]);
        continue;
      }
  ...
  if (counter_dup)
     DEBUG1 (2, "Counter Name is -> %s\n", counter_dup);
  else 
      {
        DEBUG0 (2, "Pl Specify the Counter Name\n");
        exit(2);
      }
  ...
  if ( count > 1 )
    {
       if ( refresh_rate.tv_nsec > 0  || refresh_rate.tv_sec > 2 )
                { }
       else
           {
  ...
  while (i < count )
   {
     ...
     if ( status == stop_probe)
     ...
     else if (status == start_probe )
      {
	    ...
	    ret = getdata(counter_dup, data);

	    if ( ret < 0)
		{
		  fprintf(stderr,"start_probe:- ret value < 0 \n");
		  continue;
		}
	// here if ret values is -1 , never print the data 
	// because the name of the counter is wrong.
	// So print the message and exit the program.
      ...
      else if (status == read_repeatInterval )
      {
        ...
        {
          ACE_Write_Guard<ACE_Thread_Mutex> guard(TD_asset_guard);
          if ( refresh_rate.tv_sec > 0 || refresh_rate.tv_nsec > 2 )
          { }
          else
           {
              refresh_rate.tv_sec = repeat_intrval.tv_sec;
