...
/** @file	
 *  @ingroup	RemosProbes
 *  @brief Contains implementation of a Remos Probe that delivers flow data.
 *
 * This file provides implementation of a Remos Flow Continuous Probe that 
...
// Remos specific headers
extern "C" {
#include "remos.h"
}

// local headers
#include "Remos_Flow_Probe.h"
...
/// RCS version
static const char* version = "$Id: Remos_Flow_Probe.cpp,v 1.14 2001/12/17 18:32:58 spatil Exp $";
...
worker(void *args)
{
  ...
  // local variables. 
  char *data = 0, *collhost = 0, *port = 0, duration[1024];
  ...
  for (  i = 1; args_pa[i] != 0 ; i++)
   {
     ...
     else if (ACE_OS::strcmp(args_pa[i],"-c") == 0)
      {
        collhost = (args_pa[++i]);
        continue;
      }
   ...
   strcat (collhost, " ");
   strcat (collhost,port);
   string hostport(collhost);
   string collenv ("COLLECTOR=" + hostport);
   //cout << "Setting The collector env variable -> " << collenv << endl;
   putenv(const_cast< char * >(collenv.data())); 
  ...
  if ( count > 1 )
    {
       if ( refresh_rate.tv_nsec > 0  || refresh_rate.tv_sec > 0 )
       ...
       else
           {
              ...
              DEBUG2 (2, "USING DEFAULT Secs -> %ld\t NanoSecs-> %ld\n",
  ...
  DEBUG3 (2,"PROBE Worker: count ->%d\t Secs -> %ld \t NanoSecs -> %ld\n",
  ...
  while (i < count )
   ...
   DEBUG0 (2, "PROBE Worker: Done with remos\n");
