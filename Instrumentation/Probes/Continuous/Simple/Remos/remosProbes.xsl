 ...
 ***                
 *** Filename:      remosProbes.xsl
 *** Version:       1.0
 *** Description:   XSL Transforms to generate Probe command parameters
 ***                
 *** Author:        Amarendranath Vadlamudi <amar.vadlamudi@stdc.com>
 *** Created at:    Mon Nov  5 11:30:03 2001
 *** Modified by:   Amarendranath Vadlamudi <nath@vedam>
 *** Modified at:   Wed Nov 21 15:37:06 2001
...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  xmlns:probe="http://www.stdc.com/2001/QMS/Probes/RemosProbes"
  version="$Id: remosProbes.xsl,v 1.4 2001/11/21 20:42:53 nath Exp $">
  ...
  <xsl:param name="collectorhost" select="'127.0.0.1'"/>
  <xsl:param name="collectorport" select="'8001'"/>

  <!-- Defining some easy access tags 
  The Remos Probe Schema defines the action element that controls the
  execution of the probes. This is the element we're accessing here.-->
  <xsl:variable name="action" select="/qms:QMSMessage/qms:payload/probe:remosProbe/probe:bandwidthRequest/probe:probeControl/probe:action"/>
  ...
  <xsl:template name="buildAction">
    ...
    <xsl:variable name="isRequest">
      <xsl:for-each select="qms:payload/probe:remosProbe/*[1]">
	<xsl:value-of select="name(.)='bandwidthRequest'"/>
      </xsl:for-each>
    </xsl:variable>
  ...
  <xsl:template name="buildExecString">
    ...
    <xsl:if test="($verbose='true')">
      <xsl:text>Collector Host = </xsl:text>
      <xsl:value-of select="$collectorhost"/>
      <xsl:text>&#xA;</xsl:text>
      <xsl:text>Collector Port = </xsl:text>
      <xsl:value-of select="$collectorport"/>
      <xsl:text>&#xA;</xsl:text>
      <xsl:text>Command Line = </xsl:text>
    </xsl:if>

    <xsl:text>-c&#xA;</xsl:text><xsl:value-of select="$collectorhost"/>
    <xsl:text>&#xA;-p&#xA;</xsl:text><xsl:value-of select="$collectorport"/>
    <xsl:text>&#xA;-r&#xA;</xsl:text><xsl:value-of select="$repetitions"/>
    <xsl:text>&#xA;-i&#xA;</xsl:text><xsl:value-of select="$interval"/>
    <xsl:text>&#xA;-l&#xA;</xsl:text>
    <xsl:for-each
      select="qms:payload/probe:remosProbe/probe:bandwidthRequest/*">
  ...
  <xsl:template name="buildProbeString">
    ...
    <!-- According to the remosProbes.xsd schema, the control messages
    ...
    <xsl:variable name="isControlMessage">
      <xsl:for-each select="qms:payload/probe:remosProbe/probe:bandwidthRequest/*[1]">
    ...
    <xsl:choose>
      ...
      <xsl:otherwise>
        <!-- This is a flowRequest or a linkRequest -->
