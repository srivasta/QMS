 ...
 ***                
 *** Filename:      commandProbe.xsl
 *** Version:       1.0
 *** Description:   XSL Transforms to build Command Probe commands
 ***                
 *** Author:        Amarendranath Vadlamudi <amar.vadlamudi@stdc.com>
 *** Created at:    Thu Dec  13 11:30:03 2001
 *** Modified by:   Amarendranath Vadlamudi <nath@vedam>
 *** Modified at:   Fri Dec 14 03:51:45 2001
...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  xmlns:probe="http://www.stdc.com/2001/QMS/Probes/Continuous/CommandProbe"
  version="$Id: commandProbe.xsl,v 1.1 2001/12/14 08:54:26 nath Exp $">
  ...
  <xsl:variable
    name="payloadElement"
    select="/qms:QMSMessage/qms:payload/probe:commandProbe"/>
  <xsl:variable
    name="action"
    select="$payloadElement/probe:probeRequest/probe:probeControl/probe:action"/>
  ...
  <xsl:template name="buildAction">
	...
	<xsl:value-of select="name(.)='probeRequest'"/>
      </xsl:for-each>
    </xsl:variable>
  ...
  <xsl:template name="buildExecString">
    ...
    <xsl:for-each
      select="qms:payload/probe:commandProbe/probe:probeRequest/probe:probeCommand/*">
      <xsl:value-of select="."/>
      <xsl:text>&#xA;</xsl:text>
    </xsl:for-each>
  </xsl:template>
  ...
  <xsl:template name="buildProbeString">
    ...
    <xsl:choose>
      ...
      <xsl:otherwise>
        <!-- This is a request for a specific action -->
