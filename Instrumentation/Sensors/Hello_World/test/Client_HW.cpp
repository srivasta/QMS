...
// Last Modified On : Thu Dec  6 11:56:09 2001
 ...
 *
 * $Id: Client_HW.cpp,v 1.20 2001/12/09 17:33:46 nath Exp $
 */
/// RCS version string
  ...
  if(buffer_p)
   {
     ...
     DEBUG0 (4,"Validating the message being sent.\n ");
  ...
  ACE_TRY
    {
	 ...
	 ACE_TRY_CHECK;
      ...
      string identity;
      identity.assign("Client Looking for ");
  ...
  for (length = offer_seq.length (), i = 0; i < length; i++)
   {
	...
	continue;
     ...
     this->handle_selected_properties(offer_seq[i].properties,
				      ACE_TRY_ENV);
     cerr << "Successfully pinged the Sensor\n";
     return;
   }
...
Test_Client::create_message(long version, string &data, const char *action)
{
  ...
  /// The interval requested between messages represented in XML duration
  /// form. Value is 1.005 seconds
  string interval = "PT1.005S";
  ...
  DEBUG0 (4,"Request Message created.\n ");
 ...
 *  
 *  Use the stored information to send messages to the probe in the
 *  following order
 *
...
Test_Client::send_messages(void *arg_p)
{
  ...
  cout << "\
 This client sends the following requests to the Sensor\n\n\
 -1 Send first name and last name.  \n\
 -2 Send pause action message.\n\
 -3 Send kill action message , which is a wrong command.\n\
 -4 Send restart action message, which is a wrong command.\n\
...
";
  ...
  if(client_p->check_message(data))
   ...
   {
     cerr << "Message failed to validate" << data << endl;
     cerr << "Not sending this message, but still continuing" << endl;
   } /* end of else */
  ACE_OS::sleep (ACE_Time_Value (10));

  cout << "Sending the Request containing a WRONG command KILL\n";
  data.erase();
  client_p->create_message(version, data, "kill");
  if(client_p->check_message(data))
   {
     client_p->_supplier.send(data);
   } /* end of if(check_message(data)) */
  ...
  cout << "Sending the Request containing a WRONG command RESTART\n";
  data.erase();
  client_p->create_message(version, data, "restart");
  if(!client_p->check_message(data))
   {
     cerr << "Message failed to validate" << data << endl;
     cerr << "Sending anyway" << endl;
   } /* end of else */
  client_p->_supplier.send(data);
  ACE_OS::sleep (ACE_Time_Value (5));
  ...
  cout << "DEBUG Sending all messages done\n";
