...
// Last Modified On : Sat Dec  1 20:45:53 2001
...
"$Id: CallBack_HW.cpp,v 1.3 2001/12/04 18:42:29 spatil Exp $";
...
do_query (void * args_p)
{
  ...
  DEBUG0(DEBUG_L_ENTER, "DEBUG: do_query\n");  
  ...
  if(packed_p)
   ...
   {
     std::cerr << "Error: empty argument for thread function" << std::endl;
   } /* end of else */
}
...
CallBack (const string &query, void *arg_p)
{
  ...
  DEBUG0(DEBUG_L_ENTER, "DEBUG: CallBack\n");
  
  struct pack_args  *packed_p = new pack_args;
  packed_p->sensor_p = (STDC::QMS::Citizen::Hello_World_Impl *) arg_p;
  ...
  if(ret == -1)
   {
     ACE_OS::perror ("Thread Cretion failedfailed\n");
   } /* end of if(ret == -1) */
}
