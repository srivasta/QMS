 ...
 *
 * $Id: Client.cpp,v 1.3 2002/01/21 23:31:47 nath Exp $
 */
/// RCS version string
...
<qms:QMSMessage xmlns=\"http://www.stdc.com/2001/QMSSchema\"\n\
  ...
  <qms:Session>\n\
    ...
    <minRepeatInterval>%f</minRepeatInterval>\n\
  </qms:Session>\n\
  <qms:payload>\n\
    %s\n\
  </qms:payload>\n\
...
Test_Client::init(STDC::QMS::CORBA_Handles &handles, char * identity_p, 
              char *filename_p, int repeats, double refresh_rate, char * category_name_p, CORBA::Environment &ACE_TRY_ENV)
{
  ...
  //Initialize the repetitions here.
  this->repetitions = repeats;

  //Initialize the repeat interval here.
  this->interval = refresh_rate;
			   ...
			   CORBA::Environment &ACE_TRY_ENV)
{
  ...
  // Commented this because connect_EC has become a public method.
  ...
  ACE_TRY
    {
	 ...
	 ACE_TRY_CHECK;
      ...
      identity.assign(this->_category_name);
      identity.append(":");
 ...
 */
void
Test_Client::connect_EC(void (*callback)(const string &data, void *arg_p),
                                         void *arg_p, 
                                          CORBA_Environment &ACE_TRY_ENV)
{
  ...
  ACE_TRY
    {
      ...
      this->_consumer.register_callback(callback, arg_p);
...
Test_Client::create_message(long version, string &data, const char *action )
{
  ...
  /// Commented thsi because repetitions and interval are private members.
  ...
  int ret = 
    snprintf (message, 8192 + BUFSIZ, request_fmt, 
	      ...
	      request_id, version, this->repetitions, this->interval, payload_p);
...
Test_Client::send_messages(void *arg_p)
{
  ...
  cout << "Sending the Remos Request Message to the Remos Sensor" << endl;
