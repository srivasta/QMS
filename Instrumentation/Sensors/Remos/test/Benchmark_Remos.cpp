...
// Created On       : Fri Oct 26 09:03:11 2001
// Created On Node  : glaurung.stdc.com
// Last Modified By : Manoj Srivastava
// Last Modified On : Tue Dec  4 23:51:39 2001
...
/** @file 
 *  @ingroup RemosSensor
 *  @brief  A  Client that send the sensor through a test suite
 *
 * This is a sample client that can beused as a simple testing
 * mechanism for testing QMS sensors. 
 *
 * $Id: Benchmark_Remos.cpp,v 1.3 2001/12/09 17:57:22 nath Exp $
 */
/// RCS version string
static const char* version = 
"$Id: Benchmark_Remos.cpp,v 1.3 2001/12/09 17:57:22 nath Exp $";

#include "Client_Remos.h"
...
ACE_Thread_Mutex TD_result_guard; // protects results
/// The current count
int count = 0;
...
ACE_hrtime_t receiving[NUM_RUNS][NUM_ITERATIONS];
 ...
 */
void
Test_Client::init(STDC::QMS::CORBA_Handles &handles, char * identity_p, char *filename_p,
		  char * category_name_p, CORBA::Environment &ACE_TRY_ENV)
{
  QMS_Trace ("Test_Client::init", __LINE__, __FILE__);
  DEBUG0 (DEBUG_L_ENTER, "DEBUG:Test_Client::init \n");

  //Initializing the list of strings with hostnames
  string host_name;
  ifstream inClientFile(filename_p, ios::in);

  while ( inClientFile >> host_name )
    {
        this->hosts.push_back(host_name);
        cerr << host_name << endl;
    }
  ...
  ACE_TRY_CHECK;
  DEBUG0 (4, "DEBUG: Got supplier ID\n");
			   ...
			   CORBA::Environment &ACE_TRY_ENV)
{
  ...
  ACE_TRY_CHECK;
  cout << "Client version " << version << "initialized." << endl;
}
 ...
 *
 * Since a message that fails to parse shall not genrate any
 * diagnostics from the sensor (the sensor not having any idea about
 * theoriginator unless the message parses), we need to validate any
        ...
        if(!msg_header.QMSUniqueName.category.empty())
         {
	      ...
	      // end timer
	      {
		ACE_Write_Guard<ACE_Thread_Mutex> guard(TD_result_guard);
		if(run < NUM_RUNS && count < NUM_ITERATIONS)
		 {
		   timer.stop();
		   timer.elapsed_microseconds(receiving[run][count++]);
		 } /* end of if(run < NUM_RUNS) */		
	      }
            } 
         }
        else
         {
           cerr << "ERROR: message did not have a valid category" << std::endl
                << "\tAborting" << std::endl
                << "I got " << msg_header.QMSUniqueName.category << std::endl
                << response;
         } 
      }
     else
...
Test_Client::create_message(long version, string &data, const char *action)
{
  ...
  string interval = "PT1.0S";
  /// The interval between messages
  int repetitions = NUM_ITERATIONS;
 

  time_t this_time = time(0);
  struct tm *now_p = gmtime((const time_t *) &this_time);

  char * payload_p = payload_generator(this->_probe_schema_uri, this->hosts, action);
  int ret = 
    snprintf (message, 8192 + BUFSIZ, request_fmt, 
	      ...
	      request_id, version, repetitions, interval.c_str(), payload_p);
...
Test_Client::send_messages(void *arg_p)
{
  ...
  ACE_hrtime_t sending[NUM_RUNS][10];

  cout << "\
  Send NUM_RUNS worth of the following sequence:\n\
 -1 Send first name and last name.  \n\
 -2 Send pause action message.\n\
 -3 Send resume action message.\n\
 -4 Send pause action message.\n\
 -5 Send resume action message.\n\
 -6 Send stop action message.\n\
";
  ...
  for(run = 0; run < NUM_RUNS; run++)
   {
	...
	sending[run][i] = 0;
     ...
     for(i = 0; i < NUM_ITERATIONS; i++)
      {
	receiving[run][i] = 0;
      } /* end of for(i = 0; i < NUM_ITERATIONS; i++) */
     
     cout << "Sending the Request\n";
     client_p->create_message(version++, data, 0);
	...
	cerr << "Still continuing" << endl;
     ...
     cout << " Sending the Request containing PAUSE command \n";
     data.erase();
     client_p->create_message(version++, data, "pause");
     if(client_p->check_message(data))
      {
	{
	  ACE_Write_Guard<ACE_Thread_Mutex> guard(TD_result_guard);
	  timer.stop();
	  timer.elapsed_microseconds(sending[run][msg_num++]);
	}	
	client_p->_supplier.send(data);
      } /* end of if(check_message(data)) */
     else
	...
	cerr << "Still continuing" << endl;
     ...
     cout <<" Sending the Request containing RESUME command \n";
     data.erase();
     client_p->create_message(version++, data, "resume");
     if(client_p->check_message(data))
      {
	{
	  ACE_Write_Guard<ACE_Thread_Mutex> guard(TD_result_guard);
	...
	cerr << "Still continuing" << endl;
     ...
     cout << " Sending the Request containing STOP command \n";
     data.erase();
   ...
   } /* end of for(run = 0; run < NUM_RUNS; run++) */
  // print out the gathered data
  {
       ...
       for(j = 0; j < 10; j++)
	{
	  if(sending[i][j])
	   {
	     std::cout << "  " << j << "  :" << sending[i][j]  << std::endl;
	   } /* end of if(sending[i][j]) */
	} /* end of for(j = 0; j < 10; j++) */
       std::cout << "\t\t Receiving messages "  << std::endl;
       for(j = 0; j < NUM_ITERATIONS; j++)
	{
	  std::cout << "  " << j << "  :" << receiving[i][j]  << std::endl;
	} /* end of for(j = 0; j < NUM_ITERATIONS; j++) */
     } /* end of for(i = 0; i < NUM_RUNS; i++) */    
  }
  
  cout << "DEBUG Sending all done\n";
  DEBUG0 (6, "Sleeping for 5 secs\n ");
