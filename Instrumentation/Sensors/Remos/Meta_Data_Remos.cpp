...
// Last Modified On : Wed Dec 12 15:42:37 2001
 ...
 *
 * $Id: Meta_Data_Remos.cpp,v 1.103 2001/12/14 00:57:57 spatil Exp $
 */
...
extern STDC::QMS::Citizen::Remos_Sensor_Impl *sensor_p;
...
/// Format for the Generic response
/// Format for the Hello World response
...
request_handler(IDOM_Node *node_p, struct Request &results)
{
  ...
  if(!node_p)
   {
     std::cerr << "request_handler " << __LINE__ << __FILE__ << std::endl
...
helloWorld_handler(IDOM_Node *node_p, struct helloWorld &results)
{
  ...
  DEBUG0(2, "DEBUG: helloWorld_handler\n");

  if(!node_p)
   {
     std::cerr << "helloWorld_handler " << __LINE__ << __FILE__ << std::endl
	  << "Missing arguments" <<endl;
     throw std::invalid_argument::invalid_argument("Missing arguments");
   }
...
payload_Handler(IDOM_Node *node_p, void *arg_p)
{
  ...
  DEBUG0(2, "DEBUG: payload_Handler\n");
  bool success = false;

  struct payload_data *results_p = (struct payload_data *)arg_p;
  if(!results_p)
   {
     std::cerr << "payload_Handler " << __LINE__ << __FILE__ << std::endl
  ...
  if(!node_p)
   {
     std::cerr << "ayload_Handler " << __LINE__ << __FILE__ << std::endl
...
STDC::QMS::Citizen::Meta_Data_Remos::Meta_Data_Remos(void) :
  _supplier_id(17), msg_sequence_no_(0), _num_probes(0), 
  status(STDC::QMS::Citizen::RUN), _offer_created(false)
{
  QMS_Trace ("STDC::QMS::Citizen::Meta_Data_Remos::Meta_Data_Remos", __LINE__, __FILE__);
 ...
 * @overload
 */
STDC::QMS::Citizen::Meta_Data_Remos::Meta_Data_Remos(STDC::QMS::Citizen::Meta_Data_Remos &rhs) :
  _config(rhs._config),
 ...
 */
const STDC::QMS::Citizen::Meta_Data_Remos
STDC::QMS::Citizen::Meta_Data_Remos::operator=(STDC::QMS::Citizen::Meta_Data_Remos &copy)
{
  QMS_Trace ("STDC::QMS::Citizen::Meta_Data_Remos::operator =", __LINE__, __FILE__);
 ...
 */
void
STDC::QMS::Citizen::Meta_Data_Remos::init(STDC::QMS::sensorServiceType  &config,
 ...
 */
void
STDC::QMS::Citizen::Meta_Data_Remos::init(STDC::QMS::ProbeDefinitionType  &probe,
				    CORBA::Long supplier_id,
				    string &QMS_Schema)
{
  ACE_Write_Guard<ACE_Thread_Mutex> guard(this->asset_guard);
 ...
 */
void
STDC::QMS::Citizen::Meta_Data_Remos::cleanup(STDC::QMS::CORBA_Handles *handles_p)
{
 ...
 */
void
STDC::QMS::Citizen::Meta_Data_Remos::cleanup(void)
{
  ACE_Write_Guard<ACE_Thread_Mutex> guard(this->asset_guard);
  DEBUG0 (2, "DEBUG: STDC::QMS::Citizen::Meta_Data_Remos::cleanup\n");
 ...
 */
void
STDC::QMS::Citizen::Meta_Data_Remos::handle_query(struct query_info *request_p)
{
  QMS_Trace("STDC::QMS::Citizen::Meta_Data_Remos::handle_query", __LINE__,__FILE__);
  DEBUG0(2,"DEBUG: STDC::QMS::Citizen::Meta_Data_Remos::handle_query\n");
  int ret;
  ...
  if(this->_is_internal)
   {
     this->internal_handle_query(request_p->query);
     DEBUG0 (4, "Finished handling internal request");
     return;
	...
	if(this->_capability.generic_prop.probeType.compare("continuous") == 0)
	 {
	   ...
	   DEBUG2(2, "Path for probe = %s, key = %s",
		  this->_capability.invocation_param.probeExecutable.c_str(),
		  use_key.c_str());
	   STDC::QMS::Citizen::Continuous_Pipe_Proxy_Remos * probe_p =
	     new  STDC::QMS::Citizen::Continuous_Pipe_Proxy_Remos(this->_capability.invocation_param.probeExecutable.c_str(), arg_list, *this, use_key);
 ...
 */
void
STDC::QMS::Citizen::Meta_Data_Remos::handle_data(string key, string data)
{
  QMS_Trace("STDC::QMS::Citizen::Meta_Data_Remos::handle_data", __LINE__,__FILE__);
  DEBUG0(2,"DEBUG: STDC::QMS::Citizen::Meta_Data_Remos::handle_data\n");
  ...
  if(!this->_capability.generic_prop.dataReusable)
   {
	...
	return;
     ...
     response_stream
       ...
       << "    <description>Remos Sensor Data</description>" <<       std::endl
       << "</qms:msgDestination>" <<                                  std::endl
       ...
       << pos->second->probe_info_p->current_index++ + 1 << "</current>"  
       <<                                                             std::endl
       << "    </minRepetitions>" << std::endl << "    <minRepeatInterval>"
       << pos->second->config.Session.interval.c_str() << "</minRepeatInterval>"
     ...
     if(data_store[index])
      {
	DEBUG1 (2, "Deleting data at location %d", index);
  ...
  else
   {
	...
	return;
     ...
     response_stream
       ...
       << "      <current>" << pos->second->id++ + 1 << "</current>"<<std::endl
       << "    </minRepetitions>" << std::endl << "    <minRepeatInterval>"
       << "PT0S" << "</minRepeatInterval>"
       << std::endl << "  </qms:Session> " << std::endl << "  <qms:payload>"
       << std::endl << data << std::endl << "  </qms:payload>" <<     std::endl
       << "</qms:QMSMessage>" << std::endl
       ;
     DEBUG1(2, "DEBUG: Data to be sent is = %s", response_stream.str().c_str());
...
/**
 * @brief Handle incoming messages for built in probes
 ...
 * time, and, if the state is RUN, we send off a response.  If the
   state is PAUSE, we just go on to the next loop.
 *
 * If the action variable was not empty, we set the probes status
 * appropriately.
 */
void
STDC::QMS::Citizen::Meta_Data_Remos::internal_handle_query(const string &query)
{
  QMS_Trace("STDC::QMS::Citizen::Meta_Data_Remos::handle_query", __LINE__,__FILE__);
  DEBUG0(2,"DEBUG: STDC::QMS::Citizen::Meta_Data_Remos::handle_query\n");
  ...
  if(buffer_p)
   {
	...
	if(config.QMSUniqueName.category.empty())
	 {
	   std::cerr << "ERROR: message did not have a valid category"
		     << std::endl
		     << "\tAborting" << std::endl
		     << "I got " << config.QMSUniqueName.category
		     << std::endl
		     << query;
	 }
      }
	   ...
	   if(run)
		...
		snprintf(response_p, buffer_size, response_fmt,
	      ...
	      if(ret == -1 || (unsigned int) ret >= buffer_size)
	       {
		 std::cerr << "Buffer overflow in outgoing message, discarding"
		      << std::endl;
 ...
 */
void
STDC::QMS::Citizen::Meta_Data_Remos::add_announce_info(STDC::QMS::Citizen::Sensor_Data_Element_Remos  * param)
{
  ACE_Write_Guard<ACE_Thread_Mutex> guard(this->asset_guard);
 ...
 */
void
STDC::QMS::Citizen::Meta_Data_Remos::add_request_info(STDC::QMS::Citizen::Sensor_Data_Element_Remos * param)
{
  ACE_Write_Guard<ACE_Thread_Mutex> guard(this->asset_guard);
 ...
 */
void
STDC::QMS::Citizen::Meta_Data_Remos::add_data_info(STDC::QMS::Citizen::Sensor_Data_Element_Remos * param)
{
  ACE_Write_Guard<ACE_Thread_Mutex> guard(this->asset_guard);
 ...
 */
void
STDC::QMS::Citizen::Meta_Data_Remos::add_ec_registry(STDC::QMS::Citizen::QMS_Trader_ECInfo_var ec_registry)
{
  ACE_Write_Guard<ACE_Thread_Mutex> guard(this->asset_guard);
 ...
 */
void
STDC::QMS::Citizen::Meta_Data_Remos::clean_EC_info(void)
{
  QMS_Trace("STDC::QMS::Citizen::Meta_Data_Remos::clean_EC_info", __LINE__,
	    __FILE__);
  DEBUG0(2,
	 "DEBUG: STDC::QMS::Citizen::Meta_Data_Remos::clean_EC_info\n");

  vector<STDC::QMS::Citizen::Sensor_Data_Element_Remos *>::iterator iter;
 ...
 */
void
STDC::QMS::Citizen::Meta_Data_Remos::get_EC_info(void)
{
  QMS_Trace("STDC::QMS::Citizen::Meta_Data_Remos::get_EC_info", __LINE__,
	    __FILE__);
  DEBUG0(2, "DEBUG: STDC::QMS::Citizen::Meta_Data_Remos::get_EC_info\n");
  ...
  ACE_TRY
	...
	this->_ec_registry->getAnnounceChannels(uniq_name_p);
	 ...
	 for(i =0; i < ImportAECSeq->length() ; i++)
	  {
	    STDC::QMS::Citizen::Sensor_Data_Element_Remos * data_p =
	      new STDC::QMS::Citizen::Sensor_Data_Element_Remos();
 ...
 */
void
STDC::QMS::Citizen::Meta_Data_Remos::create_announce_suppliers(RtecEventComm::EventSourceID my_id,
				     STDC::QMS::CORBA_Handles *handles_p,
				     CORBA::Environment &ACE_TRY_ENV)
{
  QMS_Trace("STDC::QMS::Citizen::Meta_Data_Remos::create_announce_suppliers",
	    __LINE__, __FILE__);
  DEBUG0(2,
	 "DEBUG: STDC::QMS::Citizen::Meta_Data_Remos::create_announce_suppliers\n");
 ...
 */
void
STDC::QMS::Citizen::Meta_Data_Remos::create_request_consumers
(STDC::QMS::CORBA_Handles *handles_p, CORBA::Environment &ACE_TRY_ENV)
{
  QMS_Trace("STDC::QMS::Citizen::Meta_Data_Remos::create_request_consumers",
	    __LINE__, __FILE__);
  DEBUG0(2,
	 "DEBUG: STDC::QMS::Citizen::Meta_Data_Remos::create_request_consumers\n");
 ...
 */
void
STDC::QMS::Citizen::Meta_Data_Remos::create_data_suppliers(RtecEventComm::EventSourceID my_id,
				 STDC::QMS::CORBA_Handles *handles_p,
				 CORBA::Environment &ACE_TRY_ENV)
{
  QMS_Trace("STDC::QMS::Citizen::Meta_Data_Remos::create_data_suppliers", __LINE__, __FILE__);
  DEBUG0(2, "DEBUG: STDC::QMS::Citizen::Meta_Data_Remos::create_data_suppliers\n");
 ...
 */
void
STDC::QMS::Citizen::Meta_Data_Remos::connect_announce_suppliers(CORBA::Environment &ACE_TRY_ENV)
{
  QMS_Trace("STDC::QMS::Citizen::Meta_Data_Remos::connect_announce_suppliers",
	    __LINE__, __FILE__);
  DEBUG0(2,
	 "DEBUG: STDC::QMS::Citizen::Meta_Data_Remos::connect_announce_suppliers\n");
 ...
 */
void
STDC::QMS::Citizen::Meta_Data_Remos::connect_request_consumers(CORBA::Environment &ACE_TRY_ENV)
{
  QMS_Trace("STDC::QMS::Citizen::Meta_Data_Remos::connect_request_consumers",
	    __LINE__, __FILE__);
  DEBUG0(2,
	 "DEBUG: STDC::QMS::Citizen::Meta_Data_Remos::connect_request_consumers\n");
 ...
 */
void
STDC::QMS::Citizen::Meta_Data_Remos::connect_data_suppliers(CORBA::Environment &
						      ACE_TRY_ENV)
{
  QMS_Trace("STDC::QMS::Citizen::Meta_Data_Remos::connect_data_suppliers", __LINE__,
	    __FILE__);
  DEBUG0(2,
	 "DEBUG: STDC::QMS::Citizen::Meta_Data_Remos::connect_data_suppliers\n");
 ...
 */
void
STDC::QMS::Citizen::Meta_Data_Remos::disconnect_announce_suppliers(void)
{
  QMS_Trace("STDC::QMS::Citizen::Meta_Data_Remos::disconnect_announce_suppliers",
	    __LINE__, __FILE__);
  DEBUG0(2,
	 "DEBUG:STDC::QMS::Citizen::Meta_Data_Remos::disconnect_announce_suppliers\n");
  if(!Announcement_Suppliers.empty())
   {
     unsigned int i = 0;
     for(i = 0; i < Announcement_Suppliers.size(); i++)
 ...
 */
void
STDC::QMS::Citizen::Meta_Data_Remos::disconnect_request_consumers(CORBA::Environment &ACE_TRY_ENV)
{
  QMS_Trace("STDC::QMS::Citizen::Meta_Data_Remos::disconnect_request_consumers",
	    __LINE__, __FILE__);
  DEBUG0(2,
	 "DEBUG: STDC::QMS::Citizen::Meta_Data_Remos::disconnect_request_consumers\n");
  if(!Request_Consumers.empty())
   {
     ACE_TRY
       {
 ...
 */
void
STDC::QMS::Citizen::Meta_Data_Remos::disconnect_data_suppliers(void)
{
  QMS_Trace("STDC::QMS::Citizen::Meta_Data_Remos::disconnect_data_suppliers",
	    __LINE__,  __FILE__);
  DEBUG0(2,
	 "DEBUG: STDC::QMS::Citizen::Meta_Data_Remos::disconnect_data_suppliers\n");
  if(!Data_Suppliers.empty())
   {
     unsigned int i = 0;
     for(i = 0; i < Data_Suppliers.size(); i++)
 ...
 */
int
STDC::QMS::Citizen::Meta_Data_Remos::send_error(const string &error,
					  STDC::QMS::QMSMessage &config,
					  CORBA::Environment &ACE_TRY_ENV)
{
  QMS_Trace("STDC::QMS::Citizen::Meta_Data_Remos::send_error", __LINE__, __FILE__);
  DEBUG0(2,	"DEBUG: STDC::QMS::Citizen::Meta_Data_Remos::send_error\n");
  ...
  response_stream
    ...
    << "  <qms:msgSender>" <<                                         std::endl
    << "    <id>" << this->_supplier_id << "</id>" <<                 std::endl
    << "    <description>Generic Sensor Response</description>" <<    std::endl
    << "  </qms:msgSender>" <<                                        std::endl
    << "  <qms:msgDestination>"  <<                                   std::endl
    << "    <id>" << config.sender.id << "</id>" <<                   std::endl
    << "    <description>Generic Seonsor Error</description>"  <<     std::endl
    << "</qms:msgDestination>" <<                                     std::endl
    << "  <qms:QMSUniqueName> " <<                                    std::endl
    << "    <category>" 
    << this->_capability.offer_prop.QMSUniqueName.category 
 ...
 */
int
STDC::QMS::Citizen::Meta_Data_Remos::send(const string data,
				    CORBA::Environment &ACE_TRY_ENV)
{
  int ret = 0;
  QMS_Trace("STDC::QMS::Citizen::Meta_Data_Remos::send", __LINE__, __FILE__);
  DEBUG0(2,	"DEBUG: STDC::QMS::Citizen::Meta_Data_Remos::send\n");
 ...
 */
void
STDC::QMS::Citizen::Meta_Data_Remos::withdraw_offer(STDC::QMS::CORBA_Handles *
					      handles_p,
					      CORBA::Environment
					      &ACE_TRY_ENV)
{
  QMS_Trace("STDC::QMS::Citizen::Meta_Data_Remos::withdraw_offer", __LINE__,
	    __FILE__);
  DEBUG0(2,
	 "DEBUG:STDC::QMS::Citizen::Meta_Data_Remos::withdraw_offer\n");
 ...
 */
void
STDC::QMS::Citizen::Meta_Data_Remos::make_offer(CORBA::Object_ptr obj,
					  STDC::QMS::CORBA_Handles * handles_p,
					  CORBA::Environment &ACE_TRY_ENV)
{
  QMS_Trace("STDC::QMS::Citizen::Meta_Data_Remos::make_offer", __LINE__, __FILE__);
  DEBUG0(2, "DEBUG: STDC::QMS::Citizen::Meta_Data_Remos::make_offer\n");
  ACE_TRY
    {
      CosTrading::Lookup_var lookup_var =
	handles_p->get_lookup_handle(ACE_TRY_ENV);
 ...
 *
 * This method is simpler than the one used in Remos_Sensor_Impl.cpp
 ...
 */
void
STDC::QMS::Citizen::Meta_Data_Remos::yp_offer(CORBA::Object_ptr obj,
					STDC::QMS::CORBA_Handles * handles_p,
					CORBA::Environment &ACE_TRY_ENV)
  ACE_THROW_SPEC ((CORBA::SystemException))
{
  QMS_Trace("STDC::QMS::Citizen::Meta_Data_Remos::yp_offer", __LINE__, __FILE__);
  DEBUG0(2, "DEBUG: STDC::QMS::Citizen::Meta_Data_Remos::yp_offer\n");
  if(this->_offer_created)
   {
     return;
   } /* end of if(!this->_offer_created) */
 ...
 */
void
STDC::QMS::Citizen::Meta_Data_Remos::clean_announce_suppliers(void)
{
  QMS_Trace("STDC::QMS::Citizen::Meta_Data_Remos::clean_announce_suppliers",
	    __LINE__, __FILE__);
  DEBUG0(2,
	 "DEBUG:STDC::QMS::Citizen::Meta_Data_Remos::clean_announce_suppliers\n");
  if(!Announcement_Suppliers.empty())
   {
     unsigned int i = 0;
     for(i = 0; i < Announcement_Suppliers.size(); i++)
 ...
 */
void
STDC::QMS::Citizen::Meta_Data_Remos::clean_request_consumers(void)
{
  QMS_Trace("STDC::QMS::Citizen::Meta_Data_Remos::clean_request_consumers",
	    __LINE__, __FILE__);
  DEBUG0(2,
	 "DEBUG: STDC::QMS::Citizen::Meta_Data_Remos::clean_request_consumers\n");
  if(!Request_Consumers.empty())
   {
     unsigned int i = 0;
     for(i = 0; i < Request_Consumers.size(); i++)
 ...
 */
void
STDC::QMS::Citizen::Meta_Data_Remos::clean_data_suppliers(void)
{
  QMS_Trace("STDC::QMS::Citizen::Meta_Data_Remos::clean_data_suppliers",
	    __LINE__,  __FILE__);
  DEBUG0(2,
	 "DEBUG: STDC::QMS::Citizen::Meta_Data_Remos::clean_data_suppliers\n");
  if(!Data_Suppliers.empty())
   {
     unsigned int i = 0;
     for(i = 0; i < Data_Suppliers.size(); i++)
...
template class vector<STDC::QMS::Citizen::Sensor_Data_Element_Remos *>;
template STDC::QMS::Citizen::Sensor_Data_Element_Remos ** fill_n<STDC::QMS::Citizen::Sensor_Data_Element_Remos **, unsigned int, STDC::QMS::Citizen::Sensor_Data_Element_Remos *>(STDC::QMS::Citizen::Sensor_Data_Element_Remos **, unsigned int, STDC::QMS::Citizen::Sensor_Data_Element_Remos * const &);
template void fill<STDC::QMS::Citizen::Sensor_Data_Element_Remos **, STDC::QMS::Citizen::Sensor_Data_Element_Remos *>(STDC::QMS::Citizen::Sensor_Data_Element_Remos **, STDC::QMS::Citizen::Sensor_Data_Element_Remos **, STDC::QMS::Citizen::Sensor_Data_Element_Remos * const &);
...
#pragma instantiate vector<STDC::QMS::Citizen::Sensor_Data_Element_Remos *>
