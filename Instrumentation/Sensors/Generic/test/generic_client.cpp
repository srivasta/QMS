 ...
 *
 * $Id: generic_client.cpp,v 1.5 2002/01/11 19:55:14 srivasta Exp $
 */
...
main (int argc, char *argv[])
{
  ...
  DEBUG0 (28, "DEBUG: parsing arguments\n");
  for (int i = 0; i < argc; i++)
   {
	...
	cerr << argv[0] << version << endl
	     ...
	     << "\t-v             : Prints out the version"  << endl
             << "\t-q  CATEGORY     QMS Unique Name Category (mandatory)"
	     << endl
	  ;
	exit (0);
      }
  ...
  if(!qun_category)
   {
     std::cerr << "Error: Mandatory opition category missing"
	       << std::endl;
     cerr << argv[0] << version << endl
	  << "Usage: " << argv[0] << "[CORBA OPTIONS] [OPTIONS]"
	  << endl
  ...
  ACE_TRY
    {
      ...
      handles_p->init(argc, argv, "Perf_Mon_Client", ACE_TRY_ENV);
      ACE_TRY_CHECK;
	 ...
	 exit (EXIT_FAILURE);
      ...
      client_p->init(*handles_p, (char *)"QMS Probes test client", 
		     qun_category,  ACE_TRY_ENV);
      DEBUG0 (28, "DEBUG: Client intialized. Client going to run\n");
