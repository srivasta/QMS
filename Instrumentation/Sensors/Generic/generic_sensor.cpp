...
// Last Modified On : Wed Jan  9 15:08:54 2002
 ...
 *  @brief Driver for the Generic sensor
 *
 * $Id: generic_sensor.cpp,v 1.16 2002/01/09 21:23:37 srivasta Exp $
 */
...
STDC::QMS::Citizen::Generic_Sensor_Impl *sensor_p = 
    new STDC::QMS::Citizen::Generic_Sensor_Impl();
...
public:
  ...
  sensor_error_handler(STDC::QMS::Citizen::Generic_Sensor_Impl *sensor_p,
...
private:
  ...
  STDC::QMS::Citizen::Generic_Sensor_Impl *sensor_p_;
...
{
  DEBUG1 (4, "Received signal %d", signum);
  if(signum == SIGCHLD)
   {
     DEBUG0 (4, " Ignoring SIGCHLD");
     return 0;
...
main(int argc, char *argv[])
{
  ...
  std::cerr << "Generic Sensor starting up" <<std::endl;
  ...
  sensor_error_handler *signal_handler_p = 
    new sensor_error_handler(sensor_p, handles_p);
  ...
  ACE_TRY
    {
      ...
      std::cerr << "Initialized the sensor" << std::endl;
      std::cerr << "Waiting for requests" << std::endl;
